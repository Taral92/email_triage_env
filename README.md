...
The simplest way to use the Email Triage Env environment is through the `EmailTriageEnv` class:
...
try:
    ...
    email_triage_envenv = EmailTriageEnv.from_docker_image("email_triage_env-env:latest")
    ...
    for msg in messages:
        result = email_triage_envenv.step(EmailTriageAction(message=msg))
        print(f"Sent: '{msg}'")
